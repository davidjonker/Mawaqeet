...
class AKPrayerTime {
    ...
    */
    ...
    func getDatePrayerTimes(year:Int, month:Int, day:Int, latitude:Double, longitude:Double, tZone:Float)->[TimeNames: AnyObject] {
        ...
        jDate = AKPrayerTime.julianDate(year, month: month, day: day)
    ...
    private class func julianDateFromDate(date:NSDate)->Double {
        let components = GregorianCalendar.components([NSCalendarUnit.Year, NSCalendarUnit.Month, NSCalendarUnit.Day], fromDate: NSDate())
        return julianDate(components.year, month: components.month, day: components.day)
    }
    ...
    private class func julianDate(year:Int, month:Int, day:Int)->Double {
        ...
        let result1 = floor(365.25 * (Double(yyear) + 4716.0))
        let result2 = floor(30.6001 * (Double(mmonth) + 1.0))
        let result3 = Double(dday) + B - 1524.5
